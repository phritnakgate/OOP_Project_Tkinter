...
class Study:
    def __init__(self, user, refcode):
        ...
        Label(text=self.get_name(), font=self.__header_font).pack(anchor="center")
        self.__chapter_label = []
        for i in self.get_all_chapter():
            lbl = Label(text=i, font=self.__normal_font)
        ...
        for j in range(len(self.__chapter_label)):
            pos = self.__posy
            btn = Button(text="Start", font=self.__txtbox_font, command=partial(self.press_start, pos))
    ...
    def press_start(self, pos):
        ...
        print(mat)
        txtbox = Text(font=self.__txtbox_font, width=20, height=20)
